...
local Library do
    ...
    local function SafeHttpGet(url, timeout)
        ...
        if not success then
            warn(StringFormat("[scoot] Failed to fetch asset %s: %s", tostring(url), tostring(result)))
            return nil
        end
    ...
    local function EnsureAsset(path, url)
        ...
        if not success then
            warn(StringFormat("[scoot] Failed to write asset %s: %s", tostring(path), tostring(writeError)))
            return false
        end
    ...
    local CustomFont = { } do
        function CustomFont:New(Name, Weight, Style, Data)
            ...
            if isfile(JsonPath) then
                ...
                if success then
                    ...
                    warn(StringFormat("[scoot] Failed to load cached font %s: %s", Name, tostring(fontOrError)))
                end
            end
            ...
            if not faceSuccess then
                warn(StringFormat("[scoot] Failed to register font asset %s: %s", Name, tostring(faceError)))
                return nil
            end
            ...
            if not jsonSuccess then
                warn(StringFormat("[scoot] Failed to cache font metadata %s: %s", Name, tostring(jsonError)))
                return nil
            end
            ...
            if not success then
                warn(StringFormat("[scoot] Failed to instantiate font %s: %s", Name, tostring(fontOrError)))
                return nil
            end
        ...
        function CustomFont:Get(Name)
            ...
            if isfile(JsonPath) then
                ...
                if success then
                    ...
                    warn(StringFormat("[scoot] Failed to load font %s: %s", Name, tostring(fontOrError)))
                end
            end
        end
        ...
        if not Library.Font then
            ...
            if fallbackSuccess then
                ...
                warn(StringFormat("[scoot] Failed to acquire fallback font: %s", tostring(fallbackFont)))
            end
        end
    end
    ...
    Library.RefreshConfigsList = function(self, Element)
        ...
        for _, Value in listfiles(Library.Folders.Configs) do
            local FileName = StringGSub(Value, Library.Folders.Directory .. DirectorySeparator .. ConfigFolderName .. DirectorySeparator, "")
        ...
        Components.Dropdown = function(self, Data)
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if Dropdown.IsOpen then
                    ...
                    DisconnectRenderStepped()
                    RenderSteppedName = "DropdownRender" .. HttpService:GenerateGUID(false)
