...
local Library do
    ...
    Library = {
        ...
        Folders = {
            ...
            Configs = "FALLEN-STARN/Configs",
            Assets = "FALLEN-STARN/Assets",
        },
        ...
        FadeTargetCache = setmetatable({ }, {__mode = "k"}),
        ...
        CursorVisible = false,
        WindowIsOpen = false,
        CurrentWindowItems = nil,

        _dispatchersInitialized = false,
    }
    ...
    local function initializeDispatchers(self)
        if self._dispatchersInitialized then
            return
        end

        self._dispatchersInitialized = true

        self:Connect(RunService.Heartbeat, function(delta)
            local callbacks = self.HeartbeatCallbacks
            local callbackCount = #callbacks

            for index = callbackCount, 1, -1 do
                local callback = callbacks[index]
                if callback and callback(delta) == false then
                    TableRemove(callbacks, index)
                end
            end
        end, "__library_shared_heartbeat")

        self:Connect(UserInputService.InputChanged, function(input)
            local callbacks = self.InputChangedCallbacks
            local callbackCount = #callbacks

            for index = callbackCount, 1, -1 do
                local callback = callbacks[index]
                if callback and callback(input) == false then
                    TableRemove(callbacks, index)
                end
            end
        end, "__library_shared_inputchanged")
        ...
        self:Connect(UserInputService.InputBegan, function(input, gameProcessed)
            ...
            local callbacks = self.InputBeganCallbacks
            local callbackCount = #callbacks

            for index = callbackCount, 1, -1 do
                local callback = callbacks[index]
                if callback and callback(input, gameProcessed) == false then
                    TableRemove(callbacks, index)
                end
            end

            local keybinds = self.Keybinds
            for _, keybind in keybinds do
                if keybind.HandleInputBegan then
                    keybind:HandleInputBegan(input, gameProcessed)
                end
            end
        end, "__library_shared_inputbegan")

        self:Connect(UserInputService.InputEnded, function(input, gameProcessed)
            if gameProcessed then
                return
            end

            local callbacks = self.InputEndedCallbacks
            local callbackCount = #callbacks
            ...
            for _, keybind in keybinds do
                if keybind.HandleInputEnded then
                    keybind:HandleInputEnded(input, gameProcessed)
                end
            end
        end, "__library_shared_inputended")
    end

    function Library:AddHeartbeatCallback(callback)
        initializeDispatchers(self)
        TableInsert(self.HeartbeatCallbacks, callback)
        return callback
    end

    function Library:RemoveHeartbeatCallback(callback)
        for index = #self.HeartbeatCallbacks, 1, -1 do
            if self.HeartbeatCallbacks[index] == callback then
                TableRemove(self.HeartbeatCallbacks, index)
                break
            end
        end
    end

    function Library:AddInputBeganCallback(callback)
        initializeDispatchers(self)
        TableInsert(self.InputBeganCallbacks, callback)
        return callback
    end

    function Library:RemoveInputBeganCallback(callback)
        for index = #self.InputBeganCallbacks, 1, -1 do
            if self.InputBeganCallbacks[index] == callback then
                TableRemove(self.InputBeganCallbacks, index)
                break
            end
        end
    end

    function Library:AddInputEndedCallback(callback)
        initializeDispatchers(self)
        TableInsert(self.InputEndedCallbacks, callback)
        return callback
    end

    function Library:RemoveInputEndedCallback(callback)
        for index = #self.InputEndedCallbacks, 1, -1 do
            if self.InputEndedCallbacks[index] == callback then
                TableRemove(self.InputEndedCallbacks, index)
                break
            end
        end
    end

    function Library:AddInputChangedCallback(callback)
        initializeDispatchers(self)
        TableInsert(self.InputChangedCallbacks, callback)
        return callback
    end

    function Library:RemoveInputChangedCallback(callback)
        for index = #self.InputChangedCallbacks, 1, -1 do
            if self.InputChangedCallbacks[index] == callback then
                TableRemove(self.InputChangedCallbacks, index)
                break
            end
        end
    end
    ...
    function Library:SetCustomCursorEnabled(enabled)
        self.CustomCursorEnabled = enabled

        local shouldShow = enabled and self.WindowIsOpen
        local windowItems = self.CurrentWindowItems
        local mouseBackground = windowItems and windowItems["MouseBackground"]

        if mouseBackground and mouseBackground.Instance then
            if mouseBackground.Instance.Visible ~= shouldShow then
                mouseBackground.Instance.Visible = shouldShow
            end

            if shouldShow then
                local mouseLocation = UserInputService:GetMouseLocation()
                mouseBackground.Instance.Position = UDim2New(0, mouseLocation.X - 1, 0, mouseLocation.Y - 56)
            end
        end

        if self.CursorVisible ~= shouldShow then
            self.CursorVisible = shouldShow
            UserInputService.MouseIconEnabled = not shouldShow
        end
    end

    Library.SetFlags.CustomCursor = function(Value)
        Library:SetCustomCursorEnabled(Value)
        Library.Flags["CustomCursor"] = Value
    end
    ...
    local function SafeHttpGet(url, timeout)
        ...
        if timeout then
            HttpService.HttpTimeout = timeout
        end
        ...
        if not success then
            warn(StringFormat("[FALLEN-STARN] Failed to fetch asset %s: %s", tostring(url), tostring(result)))
            return nil
        end
    ...
    local function EnsureAsset(path, url)
        local content = SafeHttpGet(url, Library.HttpTimeout)
        ...
        if not success then
            warn(StringFormat("[FALLEN-STARN] Failed to write asset %s: %s", tostring(path), tostring(writeError)))
            return false
        end
    ...
    for Index, Value in Library.Folders do
        if not isfolder(Value) then
            local success, err = pcall(makefolder, Value)
            if not success then
                warn(StringFormat("[FALLEN-STARN] Failed to create folder %s: %s", tostring(Value), tostring(err)))
            end
        end
    end
    ...
    for Index, Value in Library.Images do
        ...
        local AssetPath = Library.Folders.Assets .. "/" .. ImageName

        if not isfile(AssetPath) then
            EnsureAsset(AssetPath, ImageLink)
        end
    end
    ...
    local Tween = { } do
        ...
        Tween.Create = function(self, Item, Info, Goal, IsRawItem)
            ...
            Info = Info or TweenInfoNew(Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction)
        ...
        Instances.MakeResizeable = function(self, Minimum, Maximum)
            ...
            ResizeButton:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    if not resizeToken then
                        resizeToken = Library:AddInputChangedCallback(function(input)
                            if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then
                                return
                            end
                    ...
                    Input.Changed:Connect(function()
                        if Input.UserInputState == Enum.UserInputState.End then
                            Resizing = false
                            if resizeToken then
                                Library:RemoveInputChangedCallback(resizeToken)
                                resizeToken = nil
                            end
                        end
                    end)
    ...
    local CustomFont = { } do
        function CustomFont:New(Name, Weight, Style, Data)
            ...
            if isfile(JsonPath) then
                local success, fontOrError = pcall(function()
                ...
                if success then
                    ...
                    warn(StringFormat("[FALLEN-STARN] Failed to load cached font %s: %s", Name, tostring(fontOrError)))
                end
            end

            local FontPath = Library.Folders.Assets .. "/" .. Name .. ".ttf"

            if not isfile(FontPath) then
                if not EnsureAsset(FontPath, Data.Url) then
                    return nil
                end
            end

            local FaceAsset
            local faceSuccess, faceError = pcall(function()
                FaceAsset = getcustomasset(FontPath)
            end)

            if not faceSuccess then
                warn(StringFormat("[FALLEN-STARN] Failed to register font asset %s: %s", Name, tostring(faceError)))
                return nil
            end
            ...
            if not jsonSuccess then
                warn(StringFormat("[FALLEN-STARN] Failed to cache font metadata %s: %s", Name, tostring(jsonError)))
                return nil
            end
            ...
            if not success then
                warn(StringFormat("[FALLEN-STARN] Failed to instantiate font %s: %s", Name, tostring(fontOrError)))
                return nil
            end
        ...
        function CustomFont:Get(Name)
            ...
            if isfile(JsonPath) then
                ...
                if success then
                    ...
                    warn(StringFormat("[FALLEN-STARN] Failed to load font %s: %s", Name, tostring(fontOrError)))
                end
            end
        end
        ...
        if not Library.Font then
            local fallbackSuccess, fallbackFont = pcall(function()
                return Font.fromEnum and Font.fromEnum(Enum.Font.SourceSans) or Font.new(Enum.Font.SourceSans)
            end)

            if fallbackSuccess then
                Library.Font = fallbackFont
            else
                warn(StringFormat("[FALLEN-STARN] Failed to acquire fallback font: %s", tostring(fallbackFont)))
            end
        end
    end
    ...
    Library.AddToTheme = function(self, Item, Properties)
        ...
        for Property, Value in ThemeData.Properties do
            local NewValue = type(Value) == "string" and self.Theme[Value] or Value()

            if Item[Property] ~= NewValue then
    ...
    Library.RefreshConfigsList = function(self, Element)
        ...
        for _, Value in listfiles(Library.Folders.Configs) do
            ...
            if FileName and StringLower(string.sub(FileName, -5)) == ".json" then
        ...
        local Cached = self.ConfigListCache or { }
        local IsDifferent = #Files ~= #Cached

        if not IsDifferent then
            for Index, Value in Files do
                if Value ~= Cached[Index] then
                    IsDifferent = true
                    break
                end
            end
        ...
        if IsDifferent then
            self.ConfigListCache = TableClone(Files)
            Element:Refresh(TableClone(Files))
        end
    end
        ...
        for _, Item in self.ThemeItems do
            for Property, Value in Item.Properties do
                if type(Value) == "string" and Value == Theme then
                    if Item.Item[Property] ~= Color then
                        Item.Item[Property] = Color
                    end
                elseif type(Value) == "function" then
                    local Computed = Value()

                    if Item.Item[Property] ~= Computed then
                        Item.Item[Property] = Computed
                    end
                end
            end
        end
    ...
    Library.IsMouseOverFrame = function(self, Frame, XOffset, YOffset)
        ...
        local MouseX = Mouse.X + XOffset
        local MouseY = Mouse.Y + YOffset
        local Position = Frame.AbsolutePosition
        local Size = Frame.AbsoluteSize

        if MouseX < Position.X or MouseX > Position.X + Size.X then
            return false
        end

        if MouseY < Position.Y or MouseY > Position.Y + Size.Y then
            return false
        end
    ...
    local Components = { } do
        ...
        Components.WindowPage = function(self, Data)
            ...
            function Page:Turn(Bool)
                ...
                local fadeTargets = Library.FadeTargetCache[Items["Page"].Instance]
                ...
                if not fadeTargets then
                    ...
                    Library.FadeTargetCache[Items["Page"].Instance] = fadeTargets
                end
        ...
        Components.Button = function(self, Data)
            ...
            function Button:Add(Name, Callback)
                ...
                function NewButton:Press()
                    ...
                    local pressTween = SubItems["NewButton"]:Tween(nil, {BackgroundColor3 = accentColor})
        ...
        Components.Slider = function(self, Data)
            ...
            local function cleanupInputTracking()
                if Slider._inputChangedToken then
                    Library:RemoveInputChangedCallback(Slider._inputChangedToken)
                    Slider._inputChangedToken = nil
                end
            end

            local function applyInputPosition(position)
                local sliderInstance = Items["RealSlider"].Instance
                local sliderPos = sliderInstance.AbsolutePosition
                local sliderSize = sliderInstance.AbsoluteSize
                local sizeX = (position - sliderPos.X) / sliderSize.X
                local value = ((Data.Max - Data.Min) * sizeX) + Data.Min
                Slider:Set(value)
            end

            local function ensureInputTracking()
                if Slider._inputChangedToken then
                    return
                end

                Slider._inputChangedToken = Library:AddInputChangedCallback(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end
                    ...
                    applyInputPosition(input.Position.X)
                end)
            end
        ...
        Components.Dropdown = function(self, Data)
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if Dropdown.IsOpen then
                    ...
                    if not renderCallback then
                        renderCallback = Library:AddHeartbeatCallback(function()
                            if not Dropdown.IsOpen then
                                return false
                            end

                            local realDropdown = Items["RealDropdown"].Instance
                            local dropdownPos = realDropdown.AbsolutePosition
                            local dropdownSize = realDropdown.AbsoluteSize
                    ...
                    for _, Frame in FramesToClose do
                        Frame:SetOpen(false)
                    end
                ...
                local fadeTargets = Library.FadeTargetCache[Items["OptionHolder"].Instance]
                ...
                if not fadeTargets then
                    ...
                    Library.FadeTargetCache[Items["OptionHolder"].Instance] = fadeTargets
                end
                ...
                for _, entry in fadeTargets do
                    ...
                    if type(property) == "table" then
                        for _, prop in property do
                            lastTween = Tween:FadeItem(target, prop, Bool, Library.FadeSpeed)
                        end
                    else
                        lastTween = Tween:FadeItem(target, property, Bool, Library.FadeSpeed)
                    end
                end
            ...
            function Dropdown:Add(Option)
                ...
                local OptionOutline = Instances:Create("UIStroke", {
                    Parent = OptionButton.Instance,
                    Name = "\0",
                    ...
                    Thickness = 1,
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                })  OptionOutline:AddToTheme({Color = "Accent"})

                local OptionData = {
                    Button = OptionButton,
                    Name = Option,
                    Selected = false,
                    Outline = OptionOutline,
                    Hovering = false
                }

                function OptionData:UpdateOutline()
                    if not OptionData.Outline then
                        return
                    end

                    local shouldShow = OptionData.Selected or OptionData.Hovering
                    OptionData.Outline.Instance.Transparency = shouldShow and 0 or 1
                end
                ...
                OptionData.Button:OnHoverLeave(function()
            ...
            Dropdown._closeToken = Library:AddInputBeganCallback(function(Input)
            ...
            local function cleanupDropdown()
                if Dropdown._closeToken then
                    Library:RemoveInputBeganCallback(Dropdown._closeToken)
                    Dropdown._closeToken = nil
                end
        ...
        Components.Colorpicker = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["ColorpickerWindow"] = Instances:Create("Frame", {
                    ...
                    BorderSizePixel = 2,
                    BackgroundColor3 = FromRGB(14, 17, 15)
                })  Items["ColorpickerWindow"]:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})
                ...
                AnimationsDropdown, AnimationsDropdownItems = Components:Dropdown({
                    ...
                    Callback = function(Value)
                        ...
                        if Value == "Rainbow" then
                            ...
                            local animationSpeed = AnimationSpeedSlider.Value / 25
                            local lastHue = 0

                            Library:Thread(function()
                                while task.wait() do
                                    local newHue = MathAbs(MathSin(tick() * animationSpeed))

                                    if MathAbs(newHue - lastHue) > 0.01 then
                                        lastHue = newHue
                                        local Color = FromHSV(newHue, 1, 1)
                        ...
                        elseif Value == "Fade" then
                            if KeyframeOneLabel and KeyframeTwoLabel and AnimationSpeedSlider then
                                ...
                                Library:Thread(function()
                                    while task.wait() do
                                        local newSpeed = MathAbs(MathSin(tick() * animationSpeed))

                                        if MathAbs(newSpeed - lastSpeed) > 0.01 then
                                            lastSpeed = newSpeed
                                            Colorpicker:Set(KeyframeOneColorpicker.Color:Lerp(FromRGB(0, 0, 0), newSpeed), Colorpicker.Alpha)
                                            UpdateSync(true)
                                        end
                        ...
                        elseif Value == "Fade alpha" then
                            if KeyframeOneLabel and KeyframeTwoLabel then
                                ...
                                Library:Thread(function()
                                    while task.wait() do
                                        local newAlpha = MathAbs(MathSin(tick() * animationSpeed))

                                        if MathAbs(newAlpha - lastAlpha) > 0.01 then
                                            lastAlpha = newAlpha
                                            Colorpicker:Set(Colorpicker.Color, newAlpha)
                                            UpdateSync(true)
                                        end
                        ...
                        elseif Value == "Linear" then
                            if KeyframeOneLabel and KeyframeTwoLabel then
                                ...
                                Library:Thread(function()
                                    while task.wait() do
                                        local newPosition = MathAbs(MathSin(tick() * animationSpeed))

                                        if MathAbs(newPosition - lastPosition) > 0.01 then
                                            lastPosition = newPosition
                                            local targetColor = KeyframeOneColorpicker.Color:Lerp(KeyframeTwoColorpicker.Color, newPosition)
                                            Colorpicker:Set(targetColor, Colorpicker.Alpha)
                                            UpdateSync(true)
                                        end
            ...
            local Debounce = false
            local RenderSteppedName

            local function DisconnectRenderStepped()
                if RenderSteppedName then
                    Library:Disconnect(RenderSteppedName)
                    RenderSteppedName = nil
                end
            end
            ...
            function Colorpicker:SetOpen(Bool)
                ...
                if NewTween and NewTween.Tween then
                    local CompletionName = "ColorpickerTweenComplete" .. HttpService:GenerateGUID(false)
            ...
            function Colorpicker:Set(Color, Alpha)
                ...
                Items["PaletteDragger"]:Tween(TweenInfoNew(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2New(PaletteValueX, 0, PaletteValueY, 0)})
                Items["HueDragger"]:Tween(TweenInfoNew(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2New(0, 0, HuePositionY, 0)})
                Items["AlphaDragger"]:Tween(TweenInfoNew(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2New(AlphaPositionX, 0, 0, 0)})
            ...
            local function stopDragging()
                ...
                if Colorpicker._dragInputToken then
                    Library:RemoveInputChangedCallback(Colorpicker._dragInputToken)
                    Colorpicker._dragInputToken = nil
                end
            end

            local function ensureDragListener()
                if Colorpicker._dragInputToken then
                    return
                end

                Colorpicker._dragInputToken = Library:AddInputChangedCallback(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end

                    if SlidingPalette then
                        local paletteInstance = Items["Palette"].Instance
                        local palettePos = paletteInstance.AbsolutePosition
                        local paletteSize = paletteInstance.AbsoluteSize
                        local relX = MathClamp((input.Position.X - palettePos.X) / paletteSize.X, 0, 0.99)
                        local relY = MathClamp((input.Position.Y - palettePos.Y) / paletteSize.Y, 0, 0.99)

                        Colorpicker.Saturation = 1 - relX
                        Colorpicker.Value = 1 - relY

                        Items["PaletteDragger"].Instance.Position = UDim2New(relX, 0, relY, 0)
                        Colorpicker:Update(false, true)
                    end

                    if SlidingHue then
                        local hueInstance = Items["Hue"].Instance
                        local huePos = hueInstance.AbsolutePosition
                        local hueSize = hueInstance.AbsoluteSize
                        local relY = MathClamp((input.Position.Y - huePos.Y) / hueSize.Y, 0, 0.995)
                        Colorpicker.Hue = relY
                        Items["HueDragger"].Instance.Position = UDim2New(0, 0, relY, 0)
                        Colorpicker:Update(false, true)
                    end

                    if SlidingAlpha then
                        local alphaInstance = Items["Alpha"].Instance
                        local alphaPos = alphaInstance.AbsolutePosition
                        local alphaSize = alphaInstance.AbsoluteSize
                        local relX = MathClamp((input.Position.X - alphaPos.X) / alphaSize.X, 0, 0.995)
                        Colorpicker.Alpha = relX
                        Items["AlphaDragger"].Instance.Position = UDim2New(relX, 0, 0, 0)
                        Colorpicker:Update(true, true)
                    end
                end)
            end
            ...
            Colorpicker._closeToken = Library:AddInputBeganCallback(function(Input)
                if Input.UserInputType ~= Enum.UserInputType.MouseButton1 and Input.UserInputType ~= Enum.UserInputType.Touch then
                    return
                end
        ...
        Components.Keybind = function(self, Data)
            ...
            function Keybind:SetOpen(Bool)
                ...
                if Keybind.IsOpen then
                    ...
                    DisconnectRenderStepped()
                    RenderSteppedName = "KeybindRender" .. HttpService:GenerateGUID(false)
                    Library:Connect(RunService.Heartbeat, function()
                    ...
                    for Frame in pairs(Library.OpenFrames) do
                        if Frame ~= Keybind and Frame.SetOpen then
                            TableInsert(FramesToClose, Frame)
                        end
                    end
                ...
                if NewTween and NewTween.Tween then
                    local CompletionName = "KeybindTweenComplete" .. HttpService:GenerateGUID(false)
                    Library:Connect(NewTween.Tween.Completed, function()
                        Library:Disconnect(CompletionName)
                        Finalize()
                    end, CompletionName)
            ...
            function Keybind:HandleInputBegan(Input)
            ...
            function Keybind:HandleInputEnded(Input)
                if Keybind.Value == "None" then
                    return
                end
        ...
        Components.Textbox = function(self, Data)
            ...
            function Textbox:Set(Value)
                ...
                if Data.Numeric then
                    if (not tonumber(Value)) and StringLen(tostring(Value or "")) > 0 then
                        RawValue = Textbox.Value
                    end
                end
                ...
                Textbox.Value = RawValue
                Items["Input"].Instance.Text = RawValue
                Library.Flags[Textbox.Flag] = RawValue

                if Data.Callback then
                    Library:SafeCall(Data.Callback, RawValue, Textbox)
                end
            end
        ...
        Components.Searchbox = function(self, Data)
            ...
            local searchPadding = 8
            local outerPadding = Data.Padding or 6
            local bottomPadding = Data.BottomPadding or outerPadding
            local searchSectionHeight = searchHeight + searchPadding
            local totalOptions = Data.Items and #Data.Items or 0
            local availableOptions = totalOptions > 0 and totalOptions or 4
            local visibleOptions = Data.VisibleOptions or MathClamp(availableOptions, 4, 8)
            visibleOptions = math.max(visibleOptions, 4)
            local computedHeight = outerPadding + searchSectionHeight + (visibleOptions * optionHeight) + bottomPadding
            local targetHeight = Data.Height or computedHeight
            local minHeight = Data.MinHeight or 180
            local listHeight = math.max(targetHeight, minHeight)
            ...
            local Items = { } do
                Items["Listbox"] = Instances:Create("Frame", {
                    ...
                    BorderSizePixel = 2,
                    BackgroundColor3 = FromRGB(25, 30, 26)
                })  Items["Listbox"]:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})

                Items["ListboxStroke"] = Items["Listbox"]:Border("Outline")
                ...
                Items["Search"] = Instances:Create("Frame", {
                    ...
                    Size = UDim2New(1, -(outerPadding * 2), 0, searchHeight),
                    BorderColor3 = FromRGB(12, 12, 12),
                    BorderSizePixel = 2,
                    BackgroundColor3 = FromRGB(14, 17, 15),
                    Position = UDim2New(0, outerPadding, 0, outerPadding)
                ...
                Items["RealListbox"] = Instances:Create("Frame", {
                    ...
                    ClipsDescendants = true,
                    BorderColor3 = FromRGB(12, 12, 12),
                    Size = UDim2New(1, -(outerPadding * 2), 1, -(outerPadding + searchSectionHeight + bottomPadding)),
                    SelectionGroup = true,
                    Position = UDim2New(0, outerPadding, 0, outerPadding + searchSectionHeight),
                    Selectable = true,
                    Active = true,
                    BorderSizePixel = 2,
                    BackgroundColor3 = FromRGB(30, 36, 31)
            ...
            function Dropdown:Add(Option)
                local OptionButton = Instances:Create("TextButton", {
                    ...
                    BackgroundTransparency = 1,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Size = UDim2New(1, -6, 0, optionHeight),
                    ZIndex = 1,
                    TextSize = 9,
                    BackgroundColor3 = FromRGB(255, 255, 255)
                })  OptionButton:AddToTheme({TextColor3 = "Text"})
                ...
                OptionData.Outline = Instances:Create("UIStroke", {
                    Parent = OptionButton.Instance,
                    Name = "\0",
                    Color = FromRGB(202, 243, 255),
                    Transparency = 1,
                    Thickness = 1,
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                })  OptionData.Outline:AddToTheme({Color = "Accent"})
            ...
            Items["Listbox"]:OnHoverLeave(function()
                Items["Listbox"]:ChangeItemTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})
                Items["Listbox"]:Tween(nil, {BackgroundColor3 = Library.Theme["Background"]})
            end)

            local SearchSteppedName

            Items["Input"]:Connect("Focused", function()
                SearchSteppedName = "DropdownSearch" .. HttpService:GenerateGUID(false)
                local lastSearchText = ""
                local cachedSearchText = ""
                local cachedResults = {}
                ...
                Library:Connect(RunService.Heartbeat, function()
                    ...
                    if currentText ~= lastSearchText then
                        ...
                        if currentText ~= cachedSearchText then
                            cachedSearchText = currentText
                            cachedResults = {}

                            local lowerCurrentText = StringLower(currentText)
                            for _, Value in Dropdown.Options do
                                local shouldShow = currentText == "" or StringFind(StringLower(Value.Name), lowerCurrentText)
                                cachedResults[Value] = shouldShow
                            end
                        end

                        for _, Value in Dropdown.Options do
                            Value.Button.Instance.Visible = cachedResults[Value] or false
                        end
                    end
                end, SearchSteppedName)
            end)

            Items["Input"]:Connect("FocusLost", function()
                if SearchSteppedName then
                    Library:Disconnect(SearchSteppedName)
                    SearchSteppedName = nil
                end
            end)
    ...
    Library.Watermark = function(self, Name)
        ...
        local currentFPS = 0
        local currentPing = 0
        local statsConnectionName = "WatermarkStatsUpdate"
        ...
        local executorName, executorVersion = "Unknown Executor", nil
        do
        ...
        if executorVersion and executorVersion ~= "" then
            executorDisplay = executorDisplay .. " " .. executorVersion
        end

        local function safePingRead()
        ...
        local Items = { } do
            Items["WatermarkContainer"] = Instances:Create("Frame", {
                Parent = Library.Holder.Instance,
                Name = "\0",
            ...
            Instances:Create("UIListLayout", {
                Parent = Items["WatermarkContainer"].Instance,
                Name = "\0",
                FillDirection = Enum.FillDirection.Horizontal,
                Padding = UDimNew(0, frameSpacing),
                SortOrder = Enum.SortOrder.LayoutOrder,
            ...
            Items["WatermarkText"] = Instances:Create("TextLabel", {
                Parent = Items["Watermark"].Instance,
                Name = "\0",
                FontFace = Library.Font,
                TextColor3 = FromRGB(235, 235, 235),
                BorderColor3 = FromRGB(0, 0, 0),
                Text = Name or "FALLEN-STARN",
                ...
                RichText = true,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["WatermarkText"]:AddToTheme({TextColor3 = "Accent"})

            Items["WatermarkText"]:TextBorder()

            Items["WatermarkLiner"] = Instances:Create("Frame", {
                Parent = Items["Watermark"].Instance,
                Name = "\0",
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(202, 243, 255)
            })  Items["WatermarkLiner"]:AddToTheme({BackgroundColor3 = "Accent"})

        Items["StatsWatermark"] = Instances:Create("Frame", {
            Parent = Items["WatermarkContainer"].Instance,
            Name = "\0",
            BorderColor3 = FromRGB(12, 12, 12),
            BorderSizePixel = 2,
            AutomaticSize = Enum.AutomaticSize.XY,
            BackgroundColor3 = FromRGB(14, 17, 15)
        })  Items["StatsWatermark"]:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})

        Instances:Create("UIStroke", {
            Parent = Items["StatsWatermark"].Instance,
            Name = "\0",
            Color = FromRGB(42, 49, 45),
            LineJoinMode = Enum.LineJoinMode.Miter,
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        ...
        Items["StatsText"] = Instances:Create("TextLabel", {
            Parent = Items["StatsWatermark"].Instance,
            Name = "\0",
            FontFace = Library.Font,
            TextColor3 = FromRGB(235, 235, 235),
            BorderColor3 = FromRGB(0, 0, 0),
            Text = "FPS: --     |     Ping: -- ms     |     Time: 00:00:00",
            Position = UDim2New(0, 0, 0, 2),
            BackgroundTransparency = 1,
            TextXAlignment = Enum.TextXAlignment.Left,
            BorderSizePixel = 0,
            ...
            RichText = true,
            BackgroundColor3 = FromRGB(255, 255, 255)
        })  Items["StatsText"]:AddToTheme({TextColor3 = "Text"})

        Items["StatsText"]:TextBorder()

        Items["StatsLiner"] = Instances:Create("Frame", {
            Parent = Items["StatsWatermark"].Instance,
            Name = "\0",
            Position = UDim2New(0, -5, 0, -5),
            BorderColor3 = FromRGB(0, 0, 0),
            ...
            BorderSizePixel = 0,
            BackgroundColor3 = FromRGB(202, 243, 255)
        })  Items["StatsLiner"]:AddToTheme({BackgroundColor3 = "Accent"})

        Items["InfoWatermark"] = Instances:Create("Frame", {
            Parent = Items["WatermarkContainer"].Instance,
            Name = "\0",
            BorderColor3 = FromRGB(12, 12, 12),
            BorderSizePixel = 2,
            AutomaticSize = Enum.AutomaticSize.XY,
            BackgroundColor3 = FromRGB(14, 17, 15)
        })  Items["InfoWatermark"]:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})

        Instances:Create("UIStroke", {
            Parent = Items["InfoWatermark"].Instance,
            Name = "\0",
            Color = FromRGB(42, 49, 45),
            LineJoinMode = Enum.LineJoinMode.Miter,
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        ...
        Items["InfoText"] = Instances:Create("TextLabel", {
            Parent = Items["InfoWatermark"].Instance,
            Name = "\0",
            FontFace = Library.Font,
            TextColor3 = FromRGB(235, 235, 235),
            ...
            RichText = true,
            BackgroundColor3 = FromRGB(255, 255, 255)
        })  Items["InfoText"]:AddToTheme({TextColor3 = "Text"})

        Items["InfoText"]:TextBorder()

        Items["InfoLiner"] = Instances:Create("Frame", {
            Parent = Items["InfoWatermark"].Instance,
            Name = "\0",
            Position = UDim2New(0, -5, 0, -5),
            BorderColor3 = FromRGB(0, 0, 0),
            ...
            BorderSizePixel = 0,
            BackgroundColor3 = FromRGB(202, 243, 255)
        })  Items["InfoLiner"]:AddToTheme({BackgroundColor3 = "Accent"})
        end

        local function formatSessionTime(seconds)
            local totalSeconds = MathFloor(seconds)
            local hours = MathFloor(totalSeconds / 3600)
            local minutes = MathFloor((totalSeconds % 3600) / 60)
        ...
        local function updateInfoText()
            if not Items["InfoText"] or not isRenderable() then
                return
            end

            local newText = buildInfoText()

            if newText ~= lastInfoText and newText ~= Items["InfoText"].Instance.Text then
                Items["InfoText"].Instance.Text = newText
        ...
        local function updateStatsText()
            if not Items["StatsText"] or not isRenderable() then
                return
            end

            local elapsed = MathFloor(os.clock() - sessionStart)
            local displayFPS = currentFPS > 0 and tostring(currentFPS) or "--"
            local displayPing = currentPing > 0 and tostring(currentPing) or "--"

            local newText = "FPS: " .. displayFPS .. statsSeparator .. "Ping: " .. displayPing .. " ms" .. statsSeparator .. "Time: " .. formatSessionTime(elapsed)

            if newText ~= lastStatsText and newText ~= Items["StatsText"].Instance.Text then
                Items["StatsText"].Instance.Text = newText
                lastStatsText = newText
                updateWatermarkPosition()
            end
        end

        updateWatermarkPosition = function()
            if not isRenderable() then
                return
            end

            local containerFrame = Items["WatermarkContainer"].Instance
            if not containerFrame then
                return
            end

            local windowItems = Library.CurrentWindowItems
            local windowFrame = windowItems and windowItems["Window"] and windowItems["Window"].Instance

            if not windowFrame then
                return
            end

            local basePosition = windowFrame.AbsolutePosition
            local containerSize = containerFrame.AbsoluteSize
            local offsetX = basePosition.X
            local offsetY = basePosition.Y - containerSize.Y - frameSpacing

            if offsetX ~= lastPositionX or offsetY ~= lastPositionY then
                lastPositionX = offsetX
                lastPositionY = offsetY
                containerFrame.Position = UDim2New(0, offsetX, 0, offsetY)
            end
        end

        updateWatermarkPosition()

        function Watermark:UpdateVisibility()
            local shouldShow = self.Enabled and Library.WindowIsOpen ~= false

            Items["WatermarkContainer"].Instance.Visible = shouldShow
        ...
        local function bindWindowSignals()
            Library:Disconnect("WatermarkWindowPosition")
            local windowItems = Library.CurrentWindowItems
            local windowFrame = windowItems and windowItems["Window"] and windowItems["Window"].Instance
            ...
            Library:Connect(windowFrame:GetPropertyChangedSignal("AbsolutePosition"), updateWatermarkPosition, "WatermarkWindowPosition")
        end
        ...
        Library:Connect(RunService.Heartbeat, function(delta)
            ...
            if fpsAccumulator >= fpsUpdateInterval then
                currentFPS = MathFloor((fpsFrames / fpsAccumulator) + 0.5)
                fpsAccumulator = 0
                fpsFrames = 0
            end

            if statsUpdateTimer >= statsUpdateInterval then
                statsUpdateTimer = 0

                local newPing = safePingRead()
                if newPing ~= lastPingValue then
                    currentPing = newPing
                    lastPingValue = newPing
                    updateInfoText()
                end

                local currentSecond = MathFloor(os.clock() - sessionStart)
                if currentSecond ~= lastTimeValue then
                    lastTimeValue = currentSecond
                    updateStatsText()
                end
            end
        end, statsConnectionName)
    ...
    Library.Notification = function(self, Title, Description, Duration)
        ...
        Library:Thread(function()
            ...
            Items["Liner"]:Tween(TweenInfoNew(Duration, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2New(0, 0, 0, 1)})
    ...
    Library.Window = function(self, Data)
        ...
        local Window = {
            ...
            Size = Data.Size or Data.size or UDim2New(0, 620, 0, 460),
        ...
        local Items = Components:Window({
            ...
            Items["Content"] = Instances:Create("Frame", {
                ...
                Name = "\0",
                BackgroundTransparency = 1,
                Position = UDim2New(0, 206, 0, 12),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -218, 1, -24),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })
        ...
        local function updateMousePosition()
            if Library.CustomCursorEnabled and Library.WindowIsOpen then
                local mouseLocation = UserInputService:GetMouseLocation()
                Items["MouseBackground"].Instance.Position = UDim2New(0, mouseLocation.X - 1, 0, mouseLocation.Y - 56)
            end
        end

        Library:Disconnect("CustomCursorMouseMove")
        Window._cursorMoveToken = Library:AddInputChangedCallback(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseMovement then
                updateMousePosition()
            end
        end)

        local OldSizes = setmetatable({ }, {__mode = "k"})
        ...
        function Window:SetOpen(Bool)
            ...
            for _, entry in fadeTargets do
                ...
                if type(property) == "table" then
                    for _, prop in property do
                        NewTween = Tween:FadeItem(target, prop, Bool, Library.FadeSpeed)
                    end
                else
                    NewTween = Tween:FadeItem(target, property, Bool, Library.FadeSpeed)
                end
            end
            ...
            if NewTween and NewTween.Tween then
                NewTween.Tween.Completed:Connect(finishWindowTween)
            else
                finishWindowTween()
            end
        end

        Window._menuKeybindToken = Library:AddInputBeganCallback(function(Input)
        ...
        local function runSearchFilter()
            ...
            local isQueryEmpty = query == ""

            local showTweenInfo = TweenInfoNew(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local hideTweenInfo = showTweenInfo

            for _, value in pageSearchData do
                local name = StringLower(value.Name)
                local element = value.Element
                local elementInstance = element.Instance

                local storedSize = Window:GetOldSize(element)
                if not storedSize then
                    storedSize = elementInstance.Size
                    Window:AddToOldSizes(element, storedSize)
                end

                if isQueryEmpty or StringFind(name, query) then
                    elementInstance.Visible = true
                    local showTween = element:Tween(showTweenInfo, {Size = storedSize})
                    if showTween and showTween.Tween then
                        showTween.Tween.Completed:Once(function()
                    ...
                    local hideTween = element:Tween(hideTweenInfo, {Size = UDim2New(storedSize.X.Scale, storedSize.X.Offset, 0, 0)})
                    if hideTween and hideTween.Tween then
                        hideTween.Tween.Completed:Once(function()
                            if elementInstance then
        ...
        searchConnection = Items["Input"].Instance:GetPropertyChangedSignal("Text"):Connect(runSearchFilter)

        Items["Input"].Instance.AncestryChanged:Connect(function(_, parent)
            if not parent and searchConnection then
    ...
    Library.Pages.SubPage = function(self, Data)
        ...
        if not PageItems["SubPages"] then
            ...
            if PageFrame then
                ...
                for Index, Column in self.ColumnsData do
                    if Column and Column.Instance then
                        Column:Clean()
                        self.ColumnsData[Index] = nil
                    end
                end
        ...
        local NewSubPage, _ = Components:WindowSubPage({
            Page = SubPage.Page,
            Name = SubPage.Name,
        ...
        local SettingsPage = Window:Page({Name = "Settings", SubPages = true}) do
            local ThemingSubPage = SettingsPage:SubPage({Name = "Theming", Columns = 2}) do
                local ThemesSection = ThemingSubPage:Section({Name = "Themes", Side = 1}) do
                    ...
                    for ThemeName in Library.Theme do
                        TableInsert(ThemeKeys, ThemeName)
                    end
                    ...
                    table.sort(ThemeKeys, function(a, b)
                        ...
                        local priorityA, priorityB = getPriority(a), getPriority(b)
                    ...
                    for _, ThemeName in ThemeKeys do
                        local Value = Library.Theme[ThemeName]
                        local DisplayName = ThemeName .. " :"

                        ThemesSection:Label(DisplayName):Colorpicker({
                            Name = DisplayName,
                            Flag = ThemeName .. "Theme",
                            Default = Value,
                            Callback = function(Value)
                                Library.Theme[ThemeName] = Value
                                Library:ChangeTheme(ThemeName, Value)
                            end
                        })
                    end
                end
            ...
            local ConfigsSubPage = SettingsPage:SubPage({Name = "Configs", Columns = 2}) do
                local ConfigsSection = ConfigsSubPage:Section({Name = "Configs", Side = 1}) do
                    ...
                    ConfigsSection:Textbox({
                        Name = "Config name",
                        Default = "FALLEN-STARN",
                        Flag = "ConfigName",
                        Placeholder = "Enter text",
                        Callback = function(Value)
                            ConfigName = Value ~= "" and Value or "FALLEN-STARN"
                        end
                    })
            ...
            local SettingsSubPage = SettingsPage:SubPage({Name = "Settings", Columns = 2}) do
                local SettingsSection = SettingsSubPage:Section({Name = "Settings", Side = 1}) do
                    ...
                    SettingsSection:Toggle({
                        Name = "Custom Cursor",
                        Flag = "CustomCursor",
                        Default = Library.CustomCursorEnabled,
                        Callback = function(Value)
                            Library:SetCustomCursorEnabled(Value)
                        end
                    })
